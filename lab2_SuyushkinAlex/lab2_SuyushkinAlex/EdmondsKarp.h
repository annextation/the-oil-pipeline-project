...
template<typename T>
bool GTNetwork::bfs(T s, T t, std::unordered_map<T, T>& parent, std::unordered_map<T, std::unordered_map<T, int>>& capacity) {
...
template<typename T>
std::vector<T> GTNetwork::getMAXFlowPath(const T& source, const T& sink, std::unordered_map<T, T>& parent) {
    std::vector<T> path;
    T v = sink;
